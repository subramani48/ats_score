...
import type { AnalysisResult, ScoreBreakdown, SectionDetected } from '../../types';
...
export class AnalyzerService {
  private detectSections(text: string): SectionDetected {
  ...
  private formattingWarnings(text: string): string[] {
  ...
  private readabilityScore(text: string): number {
    ...
    if (!sentences.length) return 5;
    const avgLen = sentences.reduce((sum, s) => sum + s.split(' ').length, 0) / sentences.length;
    if (avgLen < 20) return 15;
    if (avgLen < 30) return 12;
    if (avgLen < 40) return 8;
    return 5;
  }
  ...
  analyze(text: string, domain: string): AnalysisResult {
    if (!text?.trim()) {
      return {
        ...
        missingKeywords: [],
        keywordDensity: 0,
        sectionsDetected: this.detectSections(''),
    ...
    const lower = text.toLowerCase();
    const domainKeywords = DOMAIN_KEYWORDS[domain.toLowerCase()] ?? [];
    ...
    const achievementScore =
      ACHIEVEMENT_PATTERNS.filter(p => p.test(text)).length >= 3 ? 25 : ACHIEVEMENT_PATTERNS.some(p => p.test(text)) ? 15 : 0;
    const sections = this.detectSections(text);
    const formattingScore = Math.min(20, Object.values(sections).filter(Boolean).length * 4);
    const readabilityScore = this.readabilityScore(text);
    ...
    const warnings = this.formattingWarnings(text);
    ...
    if (!sections.skills) suggestions.push('Add a dedicated Skills section listing your technical stack clearly');
    ...
    const breakdown: ScoreBreakdown = { keywordScore, achievementScore, formattingScore, readabilityScore };

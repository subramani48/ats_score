...
export class HttpExceptionFilter implements ExceptionFilter {
  private readonly logger = new Logger(HttpExceptionFilter.name);
    ...
    // Nothing more can be sent once the response has finished (a streaming handler already ended it, or the client
    // left). Writing anyway raises an unhandled 'write after end' error that stops the whole server, so only log.
    if (response.writableEnded || response.destroyed) {
      this.logger.warn(`Error after the response was already finished (${request.method} ${request.url}): ${message}`);
      return;
    }

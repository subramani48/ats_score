...
import { AI_UNAVAILABLE_MESSAGE } from '../ai/ai-timeout';
import { PrismaService } from '../../prisma/prisma.service';
import { GeminiService } from '../ai/gemini.service';
import type { BatchAnalyzeDto } from './dto/batch-analyze.dto';

// The saved and returned result must never carry raw error text; the AI service already words its own errors safely.
const safeError = (reason: unknown) => (reason instanceof HttpException ? reason.message : AI_UNAVAILABLE_MESSAGE);
...
export class BatchService {
  ...
  async analyze(dto: BatchAnalyzeDto, userId?: string) {
    ...
    const results = await Promise.allSettled(
      dto.jobDescriptions.map(async (jdItem) => {
        ...
        return {
          ...
          error: keywordGap.status === 'rejected' ? safeError(keywordGap.reason) : null,
        };
      }),
    );
    ...
    const processedResults = results.map((r, i) =>
      r.status === 'fulfilled'
        ? r.value
        : { title: dto.jobDescriptions[i].title, error: safeError(r.reason) },
    );

...
import { assertSafeToSeed, getSeedPassword } from './seed-helpers';

// The seed must never run against a database it guessed: DATABASE_URL has to be set (npm run db:seed loads .env).
function requireDatabaseUrl(): string {
  const url = process.env.DATABASE_URL;
  if (!url) {
    console.error('DATABASE_URL is not set. Set it in backend/.env and run: npm run db:seed');
    process.exit(1);
  }
  return url;
}
...
async function main() {
  // Must run before anything touches the database.
  assertSafeToSeed(DATABASE_URL);
  console.log('Seeding database...');

  // ── Users ─────────────────────────────────────────────────────────────────
  // No password is stored in this file. Use SEED_PASSWORD, or a random one is made and shown once below.
  const { password, generated } = getSeedPassword();
  const hash = await bcrypt.hash(password, 12);
  const seededEmails = ['alice@example.com', 'bob@example.com', 'admin@atsanalyzer.com'];
  // Accounts that already exist keep their old password (the upserts below do not change it).
  const alreadyExisted = new Set(
    (await prisma.user.findMany({ where: { email: { in: seededEmails } }, select: { email: true } })).map(u => u.email),
  );
...
Bob Smith`,
  ...
  const labels: Record<string, string> = {
    'alice@example.com': 'pro tier',
    'bob@example.com': 'free tier',
    'admin@atsanalyzer.com': 'admin',
  };
  for (const email of seededEmails) {
    console.log(`  ${email} — ${labels[email]} ${alreadyExisted.has(email) ? '(already existed, password unchanged)' : '(new)'}`);
  }
  if (alreadyExisted.size < seededEmails.length) {
    console.log(
      generated
        ? `\nPassword for the new accounts (shown once, save it now): ${password}`
        : '\nThe new accounts use the password you set in SEED_PASSWORD.',
    );
  }
}
